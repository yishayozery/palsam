...
export default function MissionWizard({ data, onSubmit }: { data: WData; onSubmit: (payload: unknown) => Promise<{ error?: string; ok?: boolean }> }) {
  ...
  function addSystem() { const k = nk(); setRows((r) => [...r, { key: k, source: "system", vehicleSerialUnitId: "", externalVehicleNumber: "", externalVehicleTypeName: "", soldiers: [] }]); }
  function addExternal() { const k = nk(); setRows((r) => [...r, { key: k, source: "external", vehicleSerialUnitId: "", externalVehicleNumber: "", externalVehicleTypeName: "", soldiers: [] }]); }
  function addTemplate(tid: string) {
    ...
    setRows((r) => [...r, { key: nk(), source: "system", vehicleSerialUnitId: t.vehicleSerialUnitId, externalVehicleNumber: "", externalVehicleTypeName: "", soldiers: crew }]);
  }
  ...
  const step1ok = rows.length > 0 && rows.every((r) => (r.source === "system" ? r.vehicleSerialUnitId : r.externalVehicleNumber.trim()) && r.soldiers.length > 0);
  ...
  const canNext = step === 0 ? (!!missionDate && /^\d{2}:\d{2}$/.test(departureTime)) : step === 1 ? step1ok : true;
    ...
    <div dir="rtl" style={{ fontFamily: "system-ui, sans-serif", background: C.bg, color: C.text, minHeight: "100vh", maxWidth: 480, margin: "0 auto", padding: 16, paddingBottom: 88 }}>
      ...
      {step === 1 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {rows.length === 0 && <p style={{ color: C.muted, fontSize: 14, textAlign: "center", padding: "20px 0" }}>הוסף/י רכב אחד או יותר 👆</p>}
          {rows.map((row, ri) => (
            <div key={row.key} style={{ border: `1px solid ${C.border}`, borderRadius: 12, background: C.card, padding: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <b style={{ fontSize: 14 }}>{row.source === "external" ? "🔶" : "🚗"} רכב {ri + 1}</b>
                <button onClick={() => removeRow(row.key)} style={{ fontSize: 12, color: "#ef4444", background: "none", border: "none" }}>הסר</button>
              </div>
              {row.source === "system" ? (
      ...
      {step === 2 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {rows.map((row, ri) => (
            <div key={row.key} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12 }}>
              <b style={{ fontSize: 13 }}>{row.source === "external" ? "🔶" : "🚗"} רכב {ri + 1} — {row.source === "system" ? (vehicles.find((v) => v.id === row.vehicleSerialUnitId)?.label ?? "—") : `${row.externalVehicleTypeName || "חוץ"} ${row.externalVehicleNumber}`}</b>
              <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4 }}>
                {row.soldiers.map((s) => <span key={s.key} style={{ fontSize: 12, background: C.bg, borderRadius: 6, padding: "3px 7px" }}>{s.isDriver ? "🚗 " : ""}{s.soldierId ? soldierName(s.soldierId) : s.externalName || "חוץ"}</span>)}
              </div>
            </div>
          ))}
          ...
          {err && <p style={{ color: "#dc2626", fontSize: 14, textAlign: "center" }}>{err}</p>}
        </div>
      )}
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, maxWidth: 480, margin: "0 auto", display: "flex", gap: 8, padding: 12, background: C.bg, borderTop: `1px solid ${C.border}` }}>
        {step > 0 && <button onClick={() => setStep(step - 1)} style={{ flex: 1, padding: 14, borderRadius: 10, border: `1px solid ${C.border}`, background: C.card, color: C.text, fontSize: 15, fontWeight: 600 }}>← חזרה</button>}
        {step < 2
          ? <button onClick={() => canNext && setStep(step + 1)} disabled={!canNext} style={{ flex: 2, padding: 14, borderRadius: 10, border: "none", background: canNext ? C.accent : "#94a3b8", color: "#fff", fontSize: 15, fontWeight: 700 }}>המשך →</button>
