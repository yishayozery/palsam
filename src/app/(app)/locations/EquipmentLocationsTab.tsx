...
import { saveEquipmentLocation, deleteEquipmentLocation } from "./actions";
...
export default function EquipmentLocationsTab({
  ...
  return (
    <div className="mt-4 space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-900">
        💡 <b>מיקומי ציוד</b> ב-{holderName} — איפה הציוד פיזית כשהגדוד מופעל (אוהל מ"כ, ערמה, רכב וכו').
        רכבים מופיעים אוטומטית. מיקומים אלה נבחרים מרשימה בהחתמה/קליטה — לא כטקסט חופשי.
      </div>
      ...
      {!adding ? (
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-4 space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="שם המיקום (אוהל מ&quot;כ, ערמה צפונית...)"
            ...
            <select value={newVehicleId} onChange={(e) => setNewVehicleId(e.target.value)}
              ...
              {vehicles.map((v) => (
                <option key={v.id} value={v.id}>🚙 {v.itemName} · {v.serialNumber}</option>
              ))}
            </select>
          </div>
      ...
      {/* רכבים אוטומטיים שעדיין לא הוגדרו כמיקום */}
      {(() => {
            ...
            <div className="flex flex-wrap gap-2">
              {unlinked.map((v) => (
                <button key={v.id} onClick={async () => {
                  ...
                  + הוסף {v.itemName} {v.serialNumber}
                </button>
              ))}
            </div>
          </div>
      ...
      {locations.length === 0 ? (
        <div className="text-center text-slate-400 py-10 text-sm border border-dashed border-slate-300 rounded-lg">
          אין מיקומי ציוד מוגדרים עדיין. הוסף את הראשון.
        </div>
      ) : (
