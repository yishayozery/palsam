...
export async function createReturn(formData: FormData) {
  ...
  if (toHolderIdRaw) {
    ...
    if (!chosen || chosen.battalionId !== bId || chosen.kind !== "WAREHOUSE") throw new Error("מחסן יעד לא תקף");
    ...
    if (!found) throw new Error("לא נמצא מחסן יעד תואם");
  ...
  if (await requiresPersonalId(bId)) {
    ...
    if (!pn) {
      throw new Error("🔒 הגדוד דורש מ.א. בכל מסירה. הזן מ.א. של המקבל או בחר מקבל מקושר לחייל עם מ.א.");
    }
  }
