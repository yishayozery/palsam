...
export async function saveCountDefinition(formData: FormData) {
  ...
  if (!name) return;
  const data = { name, type, frequencyId, scopeHolderId };
  if (id) await prisma.countDefinition.update({ where: { id }, data });
  else await prisma.countDefinition.create({ data: { ...data, categoryIds: [], daysOfWeek: [] } });
...
export async function startCount(formData: FormData) {
  ...
  if (type === "WAREHOUSE") {
    const whs = await prisma.holder.findMany({ where: { type: "WAREHOUSE" } });
    holderIds = whs.map((h) => h.id);
  } else if (type === "COMPANY") {
    if (scopeHolderId) holderIds = [scopeHolderId];
    else holderIds = (await prisma.holder.findMany({ where: { type: { in: ["COMPANY", "ARMORY"] } } })).map((h) => h.id);
  } else {
    // GLOBAL — כל המחזיקים
    holderIds = (await prisma.holder.findMany({ where: { active: true } })).map((h) => h.id);
  }
  ...
  await prisma.$transaction(async (tx) => {
    const session = await tx.countSession.create({
      data: {
        definitionId,
        type,
        status: type === "GLOBAL" ? "FROZEN" : "IN_PROGRESS",
        frozen: type === "GLOBAL",
        startedById: user.id,
      },
    });
    ...
    const balances = await tx.stockBalance.findMany({
      where: { holderId: { in: holderIds }, quantity: { gt: 0 } },
    });
    ...
    // שורות סריאליות (כולל חתומים על חיילים בספירה רוחבית)
    const units = await tx.serialUnit.findMany({
      where: {
        OR: [
          { currentHolderId: { in: holderIds } },
          ...(type === "GLOBAL" ? [{ signedSoldierId: { not: null } }] : []),
        ],
      },
    });
...
/** סיום ספירה — חישוב פערים ופתיחת התראות לאישור Admin. */
      ...
      if (counted !== line.expectedQty) {
        await tx.discrepancy.create({
          data: {
            ...
            holderId: line.holderId,
            expectedQty: line.expectedQty,
            countedQty: counted,
            diff: counted - line.expectedQty,
