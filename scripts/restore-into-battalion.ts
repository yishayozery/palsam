...
async function main() {
  ...
  const backup = await p.backupRun.findFirst({ where: { status: "OK", data: { not: null } }, orderBy: { createdAt: "desc" }, select: { id: true, createdAt: true, data: true } });
  ...
  // טבלאות-ייחוס חסרות בגיבוי → תחליף ביעד
  const dstStatus = await p.itemStatus.findFirst({ where: { battalionId: dst.id, isDefault: true }, select: { id: true } });
  if (!dstStatus) throw new Error("אין סטטוס ברירת-מחדל ביעד");
  const restoreCat = await p.category.create({ data: { battalionId: dst.id, name: "שוחזר מגיבוי (בדיקה)", warehouseType: "EQUIPMENT" } });
  const subs: string[] = ["Category (חסר בגיבוי) → נוצרה קטגוריית-שחזור אחת", "ItemStatus (חסר) → שויך לסטטוס ברירת-מחדל של היעד"];
  ...
  for (const it of bySrc(t.itemTypes)) {
    const ni = await p.itemType.create({ data: { battalionId: dst.id, name: String(it.name), sku: (it.sku as string | null) ?? `RST-${Math.floor(Math.random()*1e6)}`, categoryId: restoreCat.id, trackingMethod: it.trackingMethod as never, active: it.active as boolean } });
  ...
  await p.serialUnit.createMany({ data: srcUnits.map((u) => ({
    ...
    statusId: dstStatus.id, currentHolderId: u.currentHolderId ? holderMap.get(u.currentHolderId as string) ?? null : null,
  ...
  await p.stockBalance.createMany({ data: srcStock.map((sb) => ({
    ...
    statusId: dstStatus.id, quantity: sb.quantity as number,
  ...
  const [dHolders, dItems, dSol, dUnits, dStock] = await Promise.all([
    ...
    p.itemType.count({ where: { battalionId: dst.id, categoryId: restoreCat.id } }),
  ...
  console.log(`\nℹ️ לניקוי היעד: מחק חיילים+holders+itemTypes של קטגוריית '${restoreCat.id}' בגדוד ${dstCode}.`);
}
